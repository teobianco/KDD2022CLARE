...
def load_data(dataset_name, time):
    r"""Load SNAP Community datasets and return `edges`, `communities`, and `feat` (if exists)"""
    communties = open(f"./dataset/{dataset_name}/time_{time}/{dataset_name}_{time}-1.90.cmty.txt")
    edges = open(f"./dataset/{dataset_name}/time_{time}/{dataset_name}_{time}-1.90.ungraph.txt")
    ...
    print(f"[{dataset_name.upper()} TIME: {time}] #Nodes {num_node}, #Edges {num_edges}, #Communities {num_comm}")

    node_feats = None

    return num_node, num_edges, num_comm, nodes, edges, communties, node_feats, mapping
...
def prepare_data(dataset="amazon", time=0):
    r"""Core Function: load community dataset, return numbers, `PyG` data, and `List` communities"""
    num_node, num_edge, num_community, nodes, edges, communities, features, mapping = load_data(dataset, time)
    if features is None:
    ...
    return num_node, num_edge, num_community, graph_data, nx_graph, communities, mapping
